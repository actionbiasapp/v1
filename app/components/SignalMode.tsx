...
interface SignalData {
  ...
  performance: { totalGain: number; percentageChange: number; timeframe: string };
...
export default function SignalMode({
  ...
  useEffect(() => {
    if (isEnabled && holdings.length > 0) {
      const data = calculateSignalData(holdings, displayCurrency, exchangeRates, allocationTargets);
      setSignalData(data);
    }
  }, [isEnabled, holdings, displayCurrency, exchangeRates, allocationTargets]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6 mb-6 md:mb-8">
            ...
            <div className="bg-gradient-to-br from-emerald-600 to-green-700 rounded-xl p-6">
              ...
              <div className="space-y-3">
                ...
                <div className={`text-lg font-semibold ${signalData.performance.percentageChange >= 0 ? 'text-emerald-200' : 'text-red-200'}`}>
                  {signalData.performance.percentageChange >= 0 ? '+' : ''}{signalData.performance.percentageChange.toFixed(1)}%
                </div>
                <div className="text-sm text-emerald-100">
                  {signalData.performance.timeframe}
                </div>
              </div>
            </div>
...
function calculateSignalData(
  ...
  // Calculate performance (simplified - would need historical data for real performance)
  const totalGain = 0; // Placeholder - would calculate from historical data
